...
def get_ages(some_years_of_birth):
    return [2014 - i for i in years_of_birth]
...
def get_first_n_last(some_list):
    return some_list[0::len(some_list) - 1]
...
# C сохранением порядка элементов
# def get_list_without_repetition(some_list):
#     result = []
#     for a in some_list:
#         if a not in result:
...
def count_symbols(some_string):
    return {ch: some_string.count(ch) for ch in some_string}
