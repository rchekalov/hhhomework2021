...
def even(some_not_even_list):
  return list(filter(lambda item: (item % 2) == 0, some_not_even_list))
...
def count_symbols(some_string):
  uq = list(set([char for char in some_string]))
  return dict([(char, some_string.count(char)) for char in uq])
